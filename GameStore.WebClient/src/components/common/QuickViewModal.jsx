...
const QuickViewModal = ({ isOpen, onClose, product: initialProduct }) => {
    ...
    const product = (fullProduct && fullProduct.id === initialProduct?.id) ? fullProduct : initialProduct;
    ...
    useEffect(() => {
        if (isOpen && product) {
            ...
            if (!selectedPlatform || (fullProduct && fullProduct.id === initialProduct.id && !selectedPlatform)) {
                setQuantity(1);
                const firstAvailable = product.platforms?.find(p => (product.stockByPlatform?.[p.id] ?? 0) > 0);
                    ...
                    <div className="mt-6 mb-6">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                            {product.platforms?.map(p => {
                                ...
                                const stock = product.stockByPlatform?.[p.id] ?? 0;
