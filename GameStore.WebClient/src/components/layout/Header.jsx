// GameStore.WebClient/src/components/layout/Header.jsx
  ...
  return (
    ...
    >
      <div className="relative group">
        ...
        <button
          type="submit"
          className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-hover:text-cyan-400 transition-colors"
        >
                ...
                <li key={product._id}>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="font-semibold text-sm text-gray-200 truncate group-hover:text-cyan-400 transition-colors">
                        {product.name}
                      </h4>
...
const Header = () => {
  ...
  const navLinkClass = ({ isActive }) =>
    `relative px-3 py-2 text-sm font-medium transition-colors duration-300 ${isActive ? "text-cyan-400" : "text-gray-300 hover:text-white"} group`;
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center justify-between">
          ...
          <Link to="/" className="flex items-center gap-2 group">
            ...
            <span className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-white to-gray-400 group-hover:from-cyan-400 group-hover:to-blue-400 transition-all duration-300">
              GameKey
              <span className="font-light text-gray-500 group-hover:text-gray-300">
                Store
              </span>
            </span>
          </Link>
              ...
              {isAuthenticated && user ? (
                ...
                >
                  <button className="flex items-center gap-3 p-1.5 pr-3 rounded-full bg-gray-800/50 border border-gray-700 hover:border-cyan-500/50 transition-all duration-300 group">
                    <img
                      src={`https://ui-avatars.com/api/?name=${(user?.name || "User").replace(/\s/g, "+")}&background=06b6d4&color=fff&rounded=true`}
                      alt={user.name}
                    ...
                    <HiChevronDown
                      className={`text-gray-500 group-hover:text-cyan-400 transition-transform duration-300 ${isUserMenuOpen ? "rotate-180" : ""}`}
                    />
                  </button>

                  {/* Dropdown Menu */}
                  <div
                    className={`absolute right-0 top-full pt-2 w-64 transition-all duration-300 transform origin-top-right ${
                      isUserMenuOpen
                    ...
                    <div className="bg-gray-900/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-gray-700 overflow-hidden">
                      ...
                      <div className="p-2 space-y-1">
                        {user.role === "admin" && (
                          <Link
                            to="/admin/dashboard"
                            className="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium text-cyan-400 hover:bg-cyan-500/10 transition-colors"
                          >
      ...
      {/* Mobile Menu Panel */}
      <div
        className={`md:hidden fixed inset-0 z-40 bg-gray-900/95 backdrop-blur-xl transition-transform duration-300 ${
        ...
        <div className="p-4 space-y-6 h-full overflow-y-auto pb-24">
          ...
          <nav className="space-y-2">
            <NavLink
              to="/"
              className={({ isActive }) =>
                `block px-4 py-3 rounded-xl text-lg font-medium ${isActive ? "bg-gray-800 text-cyan-400" : "text-gray-300 hover:bg-gray-800 hover:text-white"}`
              }
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  {user.role === "admin" && (
                    <Link
                      to="/admin/dashboard"
                      className="flex items-center gap-3 px-4 py-3 rounded-xl text-cyan-400 hover:bg-gray-800"
                      onClick={() => setIsMobileMenuOpen(false)}
                    >
