// GameStore.WebClient/src/features/user/userSlice.js
...
// Gửi lại mã PIN
export const resendPin = createAsyncThunk(
  "user/resendPin",
  async (emailData, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data?.message || "Gửi lại PIN thất bại",
      );
    }
  },
);
...
export const verifyResetPin = createAsyncThunk(
  ...
  async (pinData, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data?.message || "Mã PIN không hợp lệ",
      );
    }
  },
);
...
export const userSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    [login, verify, resetPassword, updatePassword, fetchUserByToken].forEach(
      (thunk) => {
        builder
