...
const CartPage = () => {
    ...
    const handleQuantityChange = (item, amount) => { // amount is +1 or -1
        ...
        const stockForPlatform = item.product.stockByPlatform?.[item.platform.id] ?? 0;
    ...
    const handleRemoveItem = (item) => {
        dispatch(removeItem({ productId: item.product.id, platformId: item.platform.id }));
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                ...
                <div className="lg:col-span-2 space-y-4">
                    {cartItems.map(item => {
                        ...
                        return (
                            <div 
                                key={`${item.product.id}-${item.platform.id}`} 
