...
const ProductDetailPage = () => {
    ...
    useEffect(() => {
        if (product) {
            ...
            const firstAvailablePlatform = product.platforms?.find(p => (product.stockByPlatform?.[p.id] ?? 0) > 0) || product.platforms?.[0];
            ...
            if (product.id) dispatch(fetchRelatedProducts(product.id));
    ...
    const stockForSelectedPlatform = product?.stockByPlatform?.[selectedPlatform?.id] ?? 0;
    const itemInCart = cartItems.find(item => item.product.id === product?.id && item.platform?.id === selectedPlatform?.id);
                                ...
                                {product.platforms && product.platforms.length > 0 ? (
                                    <div className="grid grid-cols-2 gap-3">
                                        {product.platforms.map((p) => {
                                            const platformStockCount = product.stockByPlatform?.[p.id] ?? 0;
                                    ...
                                    <div>
                                        ...
                                        <div className="flex flex-wrap gap-2 mt-2">
                                            {product.genres.map((g) => (
                                                <span key={g.id} className="px-3 py-1 bg-slate-800 rounded-lg text-sm text-cyan-300 border border-slate-700">
                                                    {g.name}
                                                </span>
                                            ))}
                                        </div>
